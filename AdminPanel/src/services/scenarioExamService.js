...
/**
 * Triggers AI auto-checking for an attempt.
 * The server evaluates every scenario with Gemini 1.5 Pro and
 * returns a DRAFT — nothing is saved to the DB yet.
 * @param {string} aId
 * @returns {{ success, data: { aiResults, warnings } }}
 */
export const aiCheckAttempt = async (aId) => {
  const { data } = await api.post(`${BASE}/attempts/${aId}/ai-check`);
  return data;
};
