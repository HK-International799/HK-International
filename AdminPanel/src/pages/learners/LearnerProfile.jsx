...
import {
  ...
  Brain, Award, FolderOpen, CalendarCheck, Activity, PackageOpen,
} from "lucide-react";
...
function AssignmentsTab({ assignments }) {
  return (
    <Card>
      <h3 className="font-semibold text-gray-700 mb-4">Assignments &amp; Assessments</h3>
      {!assignments?.length ? (
          ...
          {assignments.map((a) => (
            <div key={a._id} className="flex items-center justify-between border border-gray-100 rounded-xl p-3 text-sm">
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-[11px] px-2 py-0.5 rounded-full bg-gray-100 text-gray-500 capitalize">
                    {(a.assignmentId?.assessmentType || "general").replace(/_/g, " ")}
                  </span>
                  ...
                  {a.aiDraft?.generatedAt && (
                    <span className="text-[11px] text-indigo-500">AI Reviewed</span>
                  )}
                </div>
              </div>
              <div className="flex items-center gap-2 text-xs text-gray-500">
                ...
                {a.approvalStatus === "approved" && <Badge variant="success">Approved</Badge>}
                {a.approvalStatus === "pending" && <Badge variant="warning">Pending Approval</Badge>}
...
function DispatchTab({ learnerId }) {
  ...
  return (
    <Card>
      <h3 className="font-semibold text-gray-700 mb-4">Certificate Dispatch History</h3>
      {history === null ? (
        ...
        <div className="space-y-2">
          {history.map((h) => (
            <div key={h.certificateId} className="flex items-center justify-between border border-gray-100 rounded-xl p-3 text-sm">
              <div>
                <p className="font-medium text-gray-800 font-mono">{h.certificateNumber}</p>
                <p className="text-xs text-gray-400">
                  {h.course} · Batch {h.batch || "—"} · Tracking {h.trackingNumber || "—"}
                </p>
                {h.remarks && <p className="text-xs text-gray-400 mt-0.5">{h.remarks}</p>}
              </div>
              <div className="flex items-center gap-2 text-right">
                <span className={`inline-flex px-2.5 py-1 rounded-full text-xs font-medium ${STATUS_STYLE[h.dispatchStatus] || "bg-gray-100 text-gray-600"}`}>
                  {h.dispatchStatus?.replace("_", " ")}
                </span>
                <p className="text-xs text-gray-400">{fmtDate(h.dispatchDate)}</p>
              </div>
            </div>
          ))}
        </div>
