...
export default function Assignments() {
  ...
  const load = async () => {
    ...
    try {
      const [a, c] = await Promise.all([getAllAssignments(), getCourses()]);
      setAssignments(Array.isArray(a?.data?.assignments) ? a.data.assignments : Array.isArray(a?.assignments) ? a.assignments : Array.isArray(a) ? a : []);
      setCourses(Array.isArray(c?.data?.courses) ? c.data.courses : Array.isArray(c?.courses) ? c.courses : Array.isArray(c) ? c : []);
    } catch (err) {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (r) => {
        if (!r.dueDate) return "—";
        return (
          <span className={isOverdue(r) ? "text-red-500 font-medium flex items-center gap-1" : ""}>
      ...
      render: (r) => (
        <div className="flex gap-1">
          <button
            ...
            className="p-2 rounded-lg hover:bg-indigo-50" title="View"
          >
            <Eye size={15} className="text-indigo-600" />
          </button>
          <button
            onClick={(e) => { e.stopPropagation(); navigate(`/admin/assignments/${r._id}/submissions`); }}
            className="p-2 rounded-lg hover:bg-blue-50" title="Submissions"
          >
            <Users size={15} className="text-blue-500" />
          </button>
          <button
            onClick={(e) => { e.stopPropagation(); handleTogglePublish(r); }}
            className="p-2 rounded-lg hover:bg-emerald-50" title={r.isPublished ? "Unpublish" : "Publish"}
          >
            {r.isPublished
          ...
          <button
            ...
            className="p-2 rounded-lg hover:bg-orange-50" title="Edit"
          >
        ...
        <div className="grid md:grid-cols-5 gap-3 mb-6 overflow-x-auto">
          ...
          {courses.slice(0, 8).map((c) => (
            ...
            >
              ...
              <p className="text-xs text-gray-400">
                {assignments.filter((a) => (a.courseId?._id || a.courseId) === c._id).length} assignments
              </p>
            </div>
          ))}
        </div>
          ...
          <div className="space-y-5 max-h-[75vh] overflow-y-auto pr-1">
                  ...
                  <div className="flex-1 flex items-center justify-between">
                    <div>
                      <p className="text-sm text-gray-600">
                        Current: {editTarget.file.originalName || "Uploaded file"}
                      </p>
            ...
            <div>
              ...
              {questions.length === 0 && (
                <p className="text-xs text-gray-400 bg-gray-50 rounded-xl px-4 py-3 border border-gray-100">
                  No questions added. You can also upload a .docx file with questions instead.
                </p>
              )}
            ...
            <div className="flex justify-end gap-3 pt-2 border-t border-gray-100">
              ...
              <Button onClick={handleSave} disabled={saving}>
                {saving ? "Saving…" : editTarget ? "Save Changes" : "Create Assignment"}
              </Button>
            </div>
          </div>
        </Modal>
            ...
            <div className="space-y-5">
              {showDetail.description && (
                <div>
                  ...
                  <p className="text-gray-700 text-sm">{showDetail.description}</p>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {[
                  ...
                  { label: "Questions", value: showDetail.questions?.length ?? 0 },
                  {
                    label: "Due Date",
                  ...
                  },
                  {
                    label: "Status",
                    value: (
                      <Badge variant={showDetail.isPublished ? "success" : "warning"}>
                        {showDetail.isPublished ? "Published" : "Draft"}
                      </Badge>
                    ),
                  },
              ...
              {showDetail.file?.url && (
                <div>
                  <p className="text-xs text-gray-400 mb-1.5">Assignment File</p>
                  <a
              ...
              <div className="flex justify-end gap-2 pt-2">
                <Button
                  ...
                  onClick={() => {
                    setShowDetail(null);
                    navigate(`/admin/assignments/${showDetail._id}/submissions`);
                  }}
                >
