...
import AdminLayout from "../../components/layout/AdminLayout";
import { PageHeader, DataTable, Badge, Button, Modal, Textarea, Select, Input } from "../../components/ui";
import {
  getAllRegistrations,
  processRegistration,
...
import { ClipboardCheck, Search, CheckCircle, XCircle, Download, Filter, IndianRupee } from "lucide-react";

const statusVariant = { pending: "warning", approved: "success", rejected: "danger", completed: "primary", withdrawn: "default" };
const paymentVariant = { unpaid: "danger", partial: "warning", paid: "success", verified: "primary" };
...
export default function Registrations() {
  ...
  const [paymentForm, setPaymentForm] = useState({ paymentStatus: "paid", amount: "", paymentMode: "bank_transfer", referenceNumber: "", notes: "" });
  ...
  const openPayment = (reg) => {
    setSelected(reg);
    setPaymentForm({ paymentStatus: "paid", amount: "", paymentMode: "bank_transfer", referenceNumber: "", notes: "" });
    setShowPayment(true);
  };
  ...
  const columns = [
    ...
    { key: "institute", label: "Institute", render: (r) => r.partnerInstitute?.name || <span className="text-gray-400">Direct</span> },
    { key: "status", label: "Status", render: (r) => <Badge variant={statusVariant[r.status]}>{r.status}</Badge> },
    { key: "payment", label: "Payment", render: (r) => <Badge variant={paymentVariant[r.paymentStatus] || "default"}>{r.paymentStatus || "unpaid"}</Badge> },
    { key: "lms", label: "LMS Access", render: (r) => r.lmsAccessGranted ? <Badge variant="success">Granted</Badge> : <Badge variant="default">No</Badge> },
    { key: "orientation", label: "Orientation", render: (r) => r.orientationCompleted ? <Badge variant="success">Done</Badge> : <Badge variant="default">Pending</Badge> },
    { key: "quiz", label: "Quiz", render: (r) => r.quizPassed ? <Badge variant="success">{r.quizScore}%</Badge> : <Badge variant="default">—</Badge> },
    { key: "date", label: "Date", render: (r) => new Date(r.createdAt).toLocaleDateString() },
    { key: "actions", label: "", render: (r) => (
      <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
        {r.paymentStatus !== "verified" && (
          <button onClick={() => openPayment(r)} className="p-1.5 rounded-lg hover:bg-indigo-50" title="Confirm Payment"><IndianRupee size={16} className="text-indigo-600" /></button>
        )}
        {r.status === "pending" && (
          <>
            <button onClick={() => openAction(r, "approved")} className="p-1.5 rounded-lg hover:bg-emerald-50" title="Approve"><CheckCircle size={16} className="text-emerald-600" /></button>
            <button onClick={() => openAction(r, "rejected")} className="p-1.5 rounded-lg hover:bg-red-50" title="Reject"><XCircle size={16} className="text-red-500" /></button>
          </>
        )}
      </div>
    ) },
  ...
  const pendingCount = registrations.filter(r => r.status === "pending").length;
  const paymentPendingCount = registrations.filter(r => !r.paymentStatus || r.paymentStatus === "unpaid" || r.paymentStatus === "partial").length;
  const lmsActivatedCount = registrations.filter(r => r.lmsAccessGranted).length;

  return (
    <AdminLayout>
      <div className="space-y-6">
        <PageHeader title="Registrations" subtitle={`${total} total · ${pendingCount} pending approval · ${paymentPendingCount} payment pending`}
          actions={<Button variant="outline" onClick={handleExport}><Download size={16} /> Export CSV</Button>} />
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <Select value={paymentFilter} onChange={(e) => setPaymentFilter(e.target.value)}
            options={[{ value: "", label: "All Payments" }, { value: "unpaid", label: "Payment Pending" }, { value: "partial", label: "Partially Paid" }, { value: "paid", label: "Paid" }, { value: "verified", label: "Verified" }]} />
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
          {[
            ...
            { label: "Approved", value: registrations.filter(r => r.status === "approved").length, color: "text-emerald-600" },
            { label: "Rejected", value: registrations.filter(r => r.status === "rejected").length, color: "text-red-500" },
            { label: "Payment Pending", value: paymentPendingCount, color: "text-orange-600" },
            { label: "LMS Activated", value: lmsActivatedCount, color: "text-indigo-600" },
          ].map(s => (
            <div key={s.label} className="bg-white border border-gray-100 rounded-xl p-4">
              <p className="text-xs text-gray-400">{s.label}</p>
              <p className={`text-xl font-bold ${s.color}`}>{s.value}</p>
        ...
        {loading ? (
          ...
          <DataTable
            ...
            onRowClick={(r) => r.student?._id && navigate(`/admin/learners/${r.student._id}`)}
          />
        )}
        ...
        <Modal open={showPayment} onClose={() => setShowPayment(false)} title="Confirm Payment">
          <div className="space-y-4">
            {selected && (
              <div className="bg-gray-50 rounded-xl p-4 space-y-1">
                <p className="text-sm"><strong>Student:</strong> {selected.student?.name}</p>
                <p className="text-sm"><strong>Course:</strong> {selected.course?.title}</p>
              </div>
            )}
            <Select label="Payment Status" value={paymentForm.paymentStatus} onChange={(e) => setPaymentForm(f => ({ ...f, paymentStatus: e.target.value }))}
              options={[{ value: "partial", label: "Partially Paid" }, { value: "paid", label: "Fully Paid" }, { value: "verified", label: "Verified" }]} />
            <Input label="Amount Received (optional — records a payment entry)" type="number" value={paymentForm.amount} onChange={(e) => setPaymentForm(f => ({ ...f, amount: e.target.value }))} />
            <Select label="Payment Mode" value={paymentForm.paymentMode} onChange={(e) => setPaymentForm(f => ({ ...f, paymentMode: e.target.value }))}
              options={[{ value: "bank_transfer", label: "Bank Transfer" }, { value: "upi", label: "UPI" }, { value: "razorpay", label: "Razorpay" }, { value: "cash", label: "Cash" }, { value: "cheque", label: "Cheque" }, { value: "wise", label: "Wise" }, { value: "other", label: "Other" }]} />
            <Input label="Reference Number" value={paymentForm.referenceNumber} onChange={(e) => setPaymentForm(f => ({ ...f, referenceNumber: e.target.value }))} />
            <Textarea label="Notes (optional)" value={paymentForm.notes} onChange={(e) => setPaymentForm(f => ({ ...f, notes: e.target.value }))} />
            <div className="flex justify-end gap-3 pt-2">
              <Button variant="secondary" onClick={() => setShowPayment(false)}>Cancel</Button>
              <Button variant="success" onClick={handleConfirmPayment} loading={paymentLoading}>Confirm Payment</Button>
            </div>
          </div>
        </Modal>
      </div>
