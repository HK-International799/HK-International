...
import { getAllPayments, exportPaymentsCSV, deletePayment } from "../../services/financeService";
import { Plus, Download, Trash2, Eye, Globe, FileText } from "lucide-react";
...
export default function PaymentsList() {
  ...
  const [filters, setFilters] = useState({
    status: "", paymentMode: "", search: "", email: "",
    dateFrom: "", dateTo: "",
  });
  ...
  const columns = [
    {
      ...
      render: (r) => (
        <div>
          <p className="font-medium text-gray-800">{r.learnerName || r.learnerId?.name || "—"}</p>
          <p className="text-xs text-gray-400">{r.learnerEmail || r.learnerId?.email || ""}</p>
        </div>
      ),
    },
    {
      // Task 8.7: Source badge (Manual / Online) + Registered Learner badge
      key: "source",
      label: "Source",
      render: (r) => {
          ...
          <div className="flex flex-col gap-1">
            <span
              className={`inline-flex items-center gap-1 w-fit px-2 py-0.5 rounded-full text-xs font-medium ${
                isOnline ? "bg-sky-100 text-sky-700" : "bg-slate-100 text-slate-700"
              }`}
            >
              {isOnline ? <Globe size={11} /> : <FileText size={11} />}
              {isOnline ? "Online" : "Manual"}
            </span>
            <span
              className={`inline-flex items-center w-fit px-2 py-0.5 rounded-full text-[11px] font-medium ${
                r.isRegisteredLearner
                  ? "bg-green-50 text-green-700"
                  : "bg-amber-50 text-amber-700"
              }`}
            >
              {r.isRegisteredLearner ? "Registered" : "Guest"}
            </span>
          </div>
        );
      },
    },
    {
      ...
      label: "Course",
      render: (r) => (
        <span className="text-sm text-gray-600">{r.courseTitle || r.courseId?.title || "—"}</span>
      ),
    },
    {
      key: "amount",
    ...
    },
    {
      ...
      render: (r) => (
        <span className="text-sm capitalize text-gray-600">
          {(r.paymentMode || r.gateway || "").replace("_", " ")}
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      label: "Date",
      render: (r) =>
        r.date || r.paymentDate
          ? new Date(r.date || r.paymentDate).toLocaleDateString("en-GB")
          : "—",
    },
    {
      key: "status",
    ...
    },
    {
      ...
      render: (r) => (
        <div className="flex items-center gap-1">
          {/* Task 8.7: guest/unregistered online payments have no learner
              profile to open -- disable the profile link and route to the
              raw transaction detail instead. */}
          {r.isRegisteredLearner ? (
            <button
              onClick={(e) => {
                e.stopPropagation();
                navigate(`/admin/finance/learner/${r.learnerId}`);
              }}
              className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-500"
              title="View learner profile"
            >
            ...
            <button
              onClick={(e) => {
                e.stopPropagation();
                navigate(`/admin/finance/transaction/${r._id}?source=${r.source}`);
              }}
              className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400"
              title="View transaction (no learner profile -- guest payment)"
            >
              <FileText size={15} />
            </button>
          )}
      ...
      <div className="animate-fadeIn space-y-4">
        ...
        <div className="flex flex-wrap gap-3 bg-white border border-gray-100 rounded-2xl p-4">
          <Input
            ...
            className="w-56"
          />
          {/* Task 8.7: additive email search -- matches learner email across
              BOTH the manual LearnerPayment ledger and online/guest
          ...
          {(filters.search || filters.email || filters.status || filters.paymentMode || filters.dateFrom || filters.dateTo) && (
            <button
              onClick={() => {
                setFilters({ status: "", paymentMode: "", search: "", email: "", dateFrom: "", dateTo: "" });
                setPage(1);
              }}
              className="text-xs text-red-500 hover:text-red-700 px-2"
            >
