...
//       ? Math.round(
//           (summary.submittedAssignmentsCount / summary.totalAssignmentsCount) *
//             100,
//         )
//       : 0;
...
//                         label: "Submitted",
//                         value: summary.submittedAssignmentsCount,
//                         total: summary.totalAssignmentsCount,
//                         color: "bg-green-500",
//                       },
//                       {
//                         label: "Graded",
...
// ─── Course Card ──────────────────────────────────────────────────────────────
const CourseCard = ({ course, index }) => {
  // FIX: Derive progress from completedChapters/totalChapters if progress field is missing/zero
  const totalChapters = course.totalChapters ?? course.totalLessons ?? 0;
  const completedChapters =
    course.completedChapters ?? course.completedLessons ?? 0;
  ...
  // Determine status from progress if not provided
  const status =
    course.status ?? (progress >= 100 ? "completed" : "in_progress");
      ...
      >
        ...
        <div className="p-4">
          ...
          <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-50">
            <div className="flex items-center gap-1 text-xs text-gray-400">
              <PlayCircle className="w-3.5 h-3.5" />
              {/* FIX: Show completedChapters/totalChapters with chapter terminology */}
              {/* <span>
                {completedChapters}/{totalChapters > 0 ? totalChapters : "—"}{" "}
...
export default function Dashboard() {
  ...
  // FIX: Use enrolledCourses.length as fallback if summary count is 0 but array has items
  const enrolledCoursesCount =
    summary.enrolledCoursesCount && summary.enrolledCoursesCount > 0
        ...
        <div className="max-w-7xl mx-auto px-6 -mt-10">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            {loading ? (
              ...
              <>
                {/* FIX: Use the corrected enrolledCoursesCount */}
                <StatCard
                  icon={BookOpen}
                  label="Enrolled Courses"
                ...
                >
                  ...
                  <div className="space-y-4">
                    {enrolledCourses.slice(0, 4).map((course) => {
                      const totalChapters =
                        course.totalChapters ?? course.totalLessons ?? 0;
                      const completedChapters =
