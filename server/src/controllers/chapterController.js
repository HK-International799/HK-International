...
export const submitChapterQuiz = async (req, res) => {
  try {
    ...
    const passed = quiz.totalMarks === 0 || score >= 0; // any attempt unlocks (simple rule)

    // Mark chapter as completed in progress record
    ...
    res
      .status(500)
      .json({ message: "Error submitting quiz", error: err.message });
  }
};
